...
for (i in 1:total_seasons) {
  ...
  tryCatch({
    ...
    if (nrow(weekly_data) > 0) {
      # Data cleaning
      numeric_cols <- sapply(weekly_data, is.numeric)
      stat_cols <- names(weekly_data)[numeric_cols & !names(weekly_data) %in% c("season", "week", "games")]
      weekly_data[stat_cols][is.na(weekly_data[stat_cols])] <- 0
      
      # Create filename
      filename <- paste0("weekly_data_", season, "_", SEASON_TYPE, ".csv")
      filepath <- file.path(OUTPUT_DIR, filename)
      
      # Export to CSV
      write.csv(weekly_data, filepath, row.names = FALSE)
